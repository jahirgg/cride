...
class CircleAdmin(admin.ModelAdmin):
    ...
    make_verified.short_description = 'Verify selected circles'

    def make_unverified(self, request, queryset):
        """Make circles unverified."""
        queryset.update(verified=False)
    make_unverified.short_description = 'Unverify selected circles'

    def download_todays_rides(self, request, queryset):
        """Return today's rides."""
        now = timezone.now()
        start = datetime(now.year, now.month, now.day, 0, 9, 9)
